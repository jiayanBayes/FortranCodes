function [f] = mixed_logit_rpsp(beta)
...
for i=1:REPTITION;
    ...
    %x10 = x(:,54); 
     %%%%%%%%%%%%%%%%%%%%%%%%%%%
    nx = [x1, x2, x3];
    index = nx * beta(1:38) + beta(39) * (x4 + x5) + ...
    		BROOK .* (x6 + x7 + x8) + beta(40) * (SP.*(x6 + x7 + x8)) + ...
    		beta(5) * beta(41)* (RP .* x9) + ...
    		beta(30) * beta(41) * (SP .* x9) +...
    		BROOK .* HRANDN_SCALE_BR(:,i) + ...
    		beta(42) * (BROOK .* HRANDN_SCALE_SP(:,i)) + ...
    		beta(43) * (CALPOLY .* HRANDN_SCALE_CAL(:,i));
